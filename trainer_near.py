...
class Trainer:
    ...
    def build_data(self):
        ...
        print("few(tail) classes:", int((torch.tensor(self.cls_num_list) < 20).sum()))
        print("med classes:", int(((torch.tensor(self.cls_num_list) >= 20) & (torch.tensor(self.cls_num_list) <= 100)).sum()))
        print("many(head) classes:", int((torch.tensor(self.cls_num_list) > 100).sum()))
    ...
    def train(self):
        ...
        # ---- optional text prior regularizer (training-time) ----
        self.text_reg_lambda = float(getattr(cfg, "TEXT_REG_LAMBDA", 0.0))
        self.text_reg_T = float(getattr(cfg, "TEXT_REG_T", 1.0))

        if self.text_reg_lambda > 0:
            if not hasattr(self, "text_prior_weight"):
                print("⚠️ TEXT_REG_LAMBDA > 0 but no text_prior_weight found. Disable text regularizer.")
                self.text_reg_lambda = 0.0
            else:
                if not hasattr(self, "text_reg_loss"):
                    self.text_reg_loss = LogitKDLoss(T=self.text_reg_T)
                print(f"→ Using text prior regularizer (KL): lambda={self.text_reg_lambda}, T={self.text_reg_T}")
        
        # ---- Nearest-class probe (PRE-training) ----
        self.find_nearest_classes(
            mode="prompt",          # "prompt" or "hybrid"
            num_samples=136,
            seed=0,
            restrict_to="few",      # "all"|"few"|"med"|"many"
            save_path=os.path.join(self.cfg.output_dir, "nearest_prompt_few.json"),
            print_topk=1,
        )
        ...
        self.find_nearest_classes(
            ...
            seed=0,
            restrict_to="few",
            save_path=os.path.join(self.cfg.output_dir, "nearest_hybrid_few.json"),
            print_topk=1
        )
            ...
            for batch_idx, (image, label) in enumerate(self.train_loader):
                ...
                if cfg.prec_train == "amp":
                    with torch.autocast(device_type="cuda"):
                        ...
                        if getattr(self, "text_reg_lambda", 0.0) > 0:
                            # teacher logits (no grad)
                            with torch.no_grad():
                                text_logit = self._compute_text_prior_logits(image)
                            reg_loss = self.text_reg_loss(logit, text_logit)
                            loss = ce_loss + self.text_reg_lambda * reg_loss
                        else:
                            reg_loss = None
                            loss = ce_loss
                ...
                if reg_loss is not None:
                    tb_writer.add_scalar("train/kd_loss", reg_loss.item(), iter_idx)
    ...
    @torch.no_grad()
    def _get_class_text_embeddings(self, mode: str = "prompt"):
        """
        Returns normalized class text embeddings [C, D].
        mode:
          - "prompt": prompt only (a photo of a {CLASS}.)
          - "caption": caption only (your wiki caption feature)
          - "hybrid": hybrid feature (whatever you use for init; here we reuse _compute_caption_features result)
        """
        mode = mode.lower()
        if mode == "prompt":
            prompts = self.generate_class_prompts()               # tokenized
            emb = self.compute_class_features(prompts)            # [C, D]
            emb = F.normalize(emb, dim=-1)
            return emb

        elif mode in ["caption", "hybrid"]:
            # NOTE: your _compute_caption_features currently returns final mixed feature (w_final) already.
            # If you want "caption only", you need to modify _compute_caption_features to optionally return raw caption mean.
            emb = self._compute_caption_features()                # [C, D]
            emb = F.normalize(emb, dim=-1)
            return emb

        else:
            raise ValueError(f"Unknown mode={mode}. Choose from ['prompt','caption','hybrid'].")

    @torch.no_grad()
    def find_nearest_classes(
        self,
        mode: str = "prompt",
        num_samples: int = 20,
        seed: int = 0,
        restrict_to: str = "all",   # "all" | "few" | "med" | "many"
        save_path: str = None,
        print_topk: int = 5,
        batch_cols: int = 256,
    ):
        """
        For a subset of classes, find the nearest other class by cosine similarity.

        - num_samples: how many query classes to analyze (random subset).
        - restrict_to: sample queries from {all, few, med, many}.
        - batch_cols: compute similarity in chunks to reduce memory peak.
        """
        # 1) class embeddings [C, D]
        E = self._get_class_text_embeddings(mode=mode)  # normalized

        C = E.size(0)
        classnames = self.classnames

        # 2) choose candidate query indices
        rng = random.Random(seed)
        if restrict_to == "few":
            pool = self.few_classes.tolist() if hasattr(self, "few_classes") else [i for i, n in enumerate(self.cls_num_list) if n < 20]
        elif restrict_to == "med":
            pool = self.med_classes.tolist() if hasattr(self, "med_classes") else [i for i, n in enumerate(self.cls_num_list) if 20 <= n <= 100]
        elif restrict_to == "many":
            pool = self.many_classes.tolist() if hasattr(self, "many_classes") else [i for i, n in enumerate(self.cls_num_list) if n > 100]
        else:
            pool = list(range(C))

        if len(pool) == 0:
            raise RuntimeError(f"Empty pool for restrict_to={restrict_to}")

        if num_samples > len(pool):
            num_samples = len(pool)

        query_ids = rng.sample(pool, k=num_samples)
        query_ids.sort()

        # 3) for each query, find nearest by cosine sim (exclude self)
        results = []
        E_t = E.t()  # [D, C]

        for q in query_ids:
            q_vec = E[q:q+1]  # [1, D]
            # compute sim in chunks to reduce memory
            best = []  # list of (sim, idx)
            for start in range(0, C, batch_cols):
                end = min(C, start + batch_cols)
                sims = (q_vec @ E[start:end].t()).squeeze(0)  # [end-start]
                # mask self if in this chunk
                if start <= q < end:
                    sims[q - start] = -1e9
                # take local topk
                k = min(print_topk, sims.numel())
                top_s, top_i = torch.topk(sims, k=k, largest=True)
                for s, i in zip(top_s.tolist(), top_i.tolist()):
                    best.append((s, start + i))

            # global topk among collected candidates
            best.sort(key=lambda x: x[0], reverse=True)
            best = best[:print_topk]

            nearest_sim, nearest_idx = best[0]
            res = {
                "query_id": int(q),
                "query_class": classnames[q],
                "nearest_id": int(nearest_idx),
                "nearest_class": classnames[nearest_idx],
                "nearest_sim": float(nearest_sim),
                "topk": [
                    {"id": int(i), "class": classnames[i], "sim": float(s)} for (s, i) in best
                ],
                "not_prompt": f"a photo of a {classnames[q].replace('_',' ')}, not a {classnames[nearest_idx].replace('_',' ')}."
            }
            results.append(res)

        # 4) print nicely
        print(f"\n[Nearest-class analysis] mode={mode}, restrict_to={restrict_to}, samples={len(results)}")
        for r in results:
            qc = r["query_class"]
            nc = r["nearest_class"]
            sim = r["nearest_sim"]
            print(f"- {qc}  ->  nearest: {nc}  (cos={sim:.4f})")
            print(f"  not-prompt: {r['not_prompt']}")
            if print_topk > 1:
                tops = ", ".join([f"{x['class']}({x['sim']:.3f})" for x in r["topk"]])
                print(f"  top{print_topk}: {tops}")
