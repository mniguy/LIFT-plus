...
class Trainer:
    ...
    def test(self):
        ...
        # --- per-class acc meters ---
        correct_per_cls = torch.zeros(self.num_classes, device=self.device, dtype=torch.long)
        total_per_cls   = torch.zeros(self.num_classes, device=self.device, dtype=torch.long)

        # --- for split tag (many/med/few) ---
        few_set  = set(self.few_classes.detach().cpu().tolist())
        med_set  = set(self.med_classes.detach().cpu().tolist())
        many_set = set(self.many_classes.detach().cpu().tolist())

        def split_name(i: int) -> str:
            if i in few_set:  return "few"
            if i in med_set:  return "med"
            if i in many_set: return "many"
            return "unknown"
        ...
        for image, label in tqdm(self.test_loader, ascii=True, desc="Testing"):
            ...
            # pred = logit.argmax(dim=1)
            # total_per_cls += torch.bincount(label.view(-1), minlength=self.num_classes)
            # correct_labels = label[pred.eq(label)]
            # correct_per_cls += torch.bincount(correct_labels.view(-1), minlength=self.num_classes)
        ...
        # -------------------------
        # correct_cpu = correct_per_cls.detach().cpu()
        # total_cpu   = total_per_cls.detach().cpu()
        # acc_cpu = (correct_cpu.float() / total_cpu.clamp(min=1).float()) * 100.0
        ...
        def _mean_acc(idxs):
            ...
            if idxs.numel() == 0:
                return float("nan")
            return acc_cpu[idxs].mean().item()

        # print("\n================ Per-class Accuracy (ALL) ================")
        # rows = []
        # for i in range(self.num_classes):
        #     name = self.classnames[i] if hasattr(self, "classnames") else str(i)
        #     n = int(total_cpu[i].item())
        #     a = float(acc_cpu[i].item()) if n > 0 else float("nan")
        #     rows.append((a, n, i, name))

        # # 낮은 acc부터 정렬 (nan은 맨 뒤)
        # rows_sorted = sorted(rows, key=lambda x: (math.isnan(x[0]), x[0], x[2]))
        # for a, n, i, name in rows_sorted:
        #     print(f"[{i:4d}] {name:30s}  n={n:5d}  acc={a:6.2f}")

        # --- print per-class accuracy (with split tag) ---
        correct_cpu = correct_per_cls.detach().cpu()
        total_cpu   = total_per_cls.detach().cpu()
        acc_cpu = (correct_cpu.float() / total_cpu.clamp(min=1).float()) * 100.0
        ...
        for i in range(self.num_classes):
            ...
            n = int(total_cpu[i].item())
            a = float(acc_cpu[i].item()) if n > 0 else float("nan")
            rows.append((a, n, i, name, split_name(i)))

        # acc 낮은 순 정렬
        rows_sorted = sorted(rows, key=lambda x: (math.isnan(x[0]), x[0], x[2]))

        print("\n================ Per-class Accuracy (lowest first) ================")
        for a, n, i, name, sp in rows_sorted[:20]:
            print(f"[{i:4d}] {name:30s}  n={n:5d}  acc={a:6.2f}  split={sp}")

        # many/med/few 평균도 같이 출력
        many_acc = _mean_acc(self.many_classes)
        med_acc  = _mean_acc(self.med_classes)
        few_acc  = _mean_acc(self.few_classes)
        mean_acc = acc_cpu.mean().item()
        print("----------------------------------------------------------")
        print(f"mean={mean_acc:.2f} | many={many_acc:.2f} med={med_acc:.2f} few={few_acc:.2f}")

        # few(tail)만 따로 worst-k 출력 (원하면 k 조절)
        k = int(getattr(cfg, "PRINT_TAIL_WORST_K", 20))
        if isinstance(self.few_classes, torch.Tensor) and self.few_classes.numel() > 0:
            few_idxs = self.few_classes.detach().cpu().tolist()
            few_rows = [(acc_cpu[i].item(), int(total_cpu[i].item()), i, self.classnames[i]) for i in few_idxs]
            few_rows = sorted(few_rows, key=lambda x: (math.isnan(x[0]), x[0], x[2]))
            print(f"\n========== Tail(FEW) worst-{k} ==========")
            for a, n, i, name in few_rows[:k]:
                print(f"[{i:4d}] {name:30s}  n={n:5d}  acc={a:6.2f}")
        ...
        if out_dir is not None:
            ...
            with open(csv_path, "w", newline="") as f:
                w = csv.writer(f)
                w.writerow(["class_idx", "class_name", "num_samples", "num_correct", "acc"])
                for a, n, i, name, sp in rows:
                    w.writerow([i, name, n, int(correct_cpu[i].item()), f"{a:.4f}"])
            print(f"\n[Saved] per-class accuracy -> {csv_path}")
