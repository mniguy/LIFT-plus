#!/bin/bash
#
# Gating experiment on iNaturalist 2018 (hybrid init = the main method).
# iNat counterpart of scripts/run_places_gate.sh.
...
#
# iNat-specific knobs (vs Places): num_epochs 15, SIM_THRESHOLD 0.8.
# No PROMPT_MODE here — the places_* prompt templates are scene-specific and
# don't apply to iNat species labels (default is the default anyway).
#
...
# for iNat before trusting these results (iNat has 8142 classes; the lambda that
# moved Places splits will not transfer directly).
#
# Variants:
...
#
# Analyze with:
#   PYTHON scripts/analyze_places.py output/inat_gate --baseline fixed
...
COMMON_ARGS="-d inat2018 -b clip_vit_b16 -m lift+ num_epochs 15 tte True PEFT_WARMUP False \
    classifier_init hybrid SIM_THRESHOLD 0.8 \
...
run_gate () {
    local name="$1"; shift
    wait_for_gpu
    echo "=== [iNat gate kd=${LAMBDA_KD} nce=${LAMBDA_NCE}] ${name} ==="
    CUDA_VISIBLE_DEVICES=${GPU_ID} ${PYTHON} main.py \
        ${COMMON_ARGS} \
        "$@" \
        output_dir "inat_gate/${name}"
}
...
echo "=== [iNat gate] done. Analyze with: ==="
echo "    ${PYTHON} scripts/analyze_places.py output/inat_gate --baseline fixed"
