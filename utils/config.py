...
_C.PROMPT_CENTER_MODE = "global"  # I: global | group | tail | kappa | logcount | genus | genus_lex | diff_init | cascade | cascade_lex | nested | level | level_keep | taxo_kernel | blend | sum_all | cluster | hcluster | knn | std | whiten | pca ; J-controls: randdir | headonly | fewonly | perclass_rand
...
_C.PROMPT_CENTER_S = 0.92     # for mode=blend: out = O - (1-s)*mu_global - s*mu_LEVEL. s=0 is mode=global,
                              # s->1 approaches mode=level. Must be < 1 (s=1 IS mode=level, zero rows and all).
                              # yacs is type-strict: pass 0.0, not 0.
_C.PROMPT_CENTER_LEVEL = "genus"   # for mode=level/level_keep/blend: which taxonomy level supplies the group mean. global | genus | family | order | class | phylum | kingdom
